...
With these two layers of dimension reduction, the complexity of the model is greatly reduced. Thus, we can get a more interpretable model and a more accurate estimation is available with limited samples. The basis functions used here are treated as given, such as spline, wavelet, and Fourier basis. Also, we assume all the responses or the predictors share the same set of basis for simplicity. 
...
Here we use the daily half-hour electricity demand as the functional multivariate response with $d = 7$ (corresponding to 7 days in a week from Monday to Sunday). As for the predictors, we use the half-hour temperature data from Kent as the multivariate functional predictors, so that $p = 7$. The total sample size is $n = 508$, equaling to the number of weeks in the study period. 
...
From the fitting results we obtained above by using the temperature data from Kent, we've get only one latent predictor and five latent responses. Here, we visualize the effect of the latent predictor on the original functional response.
```{r, fig.height = 4, fig.width = 6, fig.align = "center"}
NRRR.plot.RegSurface(-fit.NRRR$Ag, fit.NRRR$Bg, fit.NRRR$Al, fit.NRRR$Bl, 
...
NRRR.plot.RegSurface(-fit.NRRR$Ag, fit.NRRR$Bg, fit.NRRR$Al, fit.NRRR$Bl, 
                     ...
                     method = "y_original")
```
One thing that needs attention is the sign of the estimated matrices. The components ${\bf U, V, A, B}$ in the nested reduced-rank structure is not identifiable even though as a whole the matrix ${\bf C}$ is identifiable. Since all the elements in the estimated ${\bf V}$ here is negative and we're plotting the relationship between ${\bf V^Tx}(s)$ and ${\bf y}(t)$, we need to specify the input `Ag` in function `NRRR.plot.RegSurface` as `-fit.NRRR$Ag` to avoid producing a heatmap of opposite values.
